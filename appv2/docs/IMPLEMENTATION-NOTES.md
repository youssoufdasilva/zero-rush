...
### Setup Commands
```bash
# From shadcn create URL - download project, then:
bun install
bun --bun run dev

# Or manual setup:
bunx create-next-app@latest appv2
cd appv2
bunx shadcn@latest init
# Select: gray base, amber theme, etc.
```
...
- Curated quality (manually verify `has_zero` and `is_good`)
...
2. System validates quality (has_zero, is_good)
...
```javascript
function toCanonicalSignature(puzzle) {
  const cards = puzzle.split(',');
  
  const operatorOrder = { '+': 0, '-': 1, '*': 2, '÷': 3 };
  
  return cards.sort((a, b) => {
    const opA = a.charAt(0);
    const opB = b.charAt(0);
    const numA = parseInt(a.substring(1));
    const numB = parseInt(b.substring(1));
    
    // Sort by operator first
    if (operatorOrder[opA] !== operatorOrder[opB]) {
      return operatorOrder[opA] - operatorOrder[opB];
    }
...
4. Explain sunset/sunrise concept
